...
- `cleaning.booking` for submitted bookings.
...
7. Odoo creates a `cleaning.booking` record.
...
The defaults are created in `hooks.py` through `post_init_hook`. They are only created when there are no existing `cleaning.booking.service` records.
